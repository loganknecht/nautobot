...
class SQLDurationRequestMetric(BaseRequestMetric):
    """Time spent executing SQL, measured via Django's `execute_wrapper` hook."""
    ...
    @property
    def description(self):
        return f"SQL {self.query_count} queries"
    ...
    def __enter__(self):
        ...
        for alias in connections:
            ...
            # Create a new context and wrap existing SQL Query for measurement
...
class RequestMetricMiddleware:
    ...
    Example:
    < Server-Timing: total;dur=42.71;desc="Total request duration", db;dur=9.59;desc="SQL 5 queries"
    ...
    def get_enabled_metrics(self):
        ...
        if settings.REQUEST_DB_DURATION_HEADER_ENABLED:
            enabled_metrics.append(SQLDurationRequestMetric())
